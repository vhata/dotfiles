...
echo -e "${BLUE}============================================${NC}"
echo -e "${BLUE}           Data Migration Guide            ${NC}"
echo -e "${BLUE}============================================${NC}"

echo -e "${GREEN}Here's a checklist of important data to transfer from your old laptop:${NC}"
echo -e ""
echo -e "${YELLOW}1. Personal Files${NC}"
echo -e "   □ Documents folder"
echo -e "   □ Desktop items"
echo -e "   □ Pictures and Photos"
echo -e "   □ Downloads (important ones)"
echo -e ""
echo -e "${YELLOW}2. Development Environment${NC}"
echo -e "   □ Local code repositories"
echo -e "   □ Database dumps"
echo -e "   □ Docker images/configs"
echo -e "   □ IDE settings and licenses"
echo -e "   □ .env files (secure credentials)"
echo -e ""
echo -e "${YELLOW}3. Application Data${NC}"
echo -e "   □ Browser bookmarks and extensions"
echo -e "   □ Email accounts configuration"
echo -e "   □ Design assets and fonts"
echo -e "   □ Chat history archives"
echo -e ""
echo -e "${YELLOW}4. System Configuration${NC}"
echo -e "   □ SSH keys (via Bitwarden)"
echo -e "   □ VPN configurations"
echo -e "   □ Wi-Fi networks and passwords"
echo -e "   □ Certificates (in Keychain)"
echo -e ""

echo -e "${GREEN}Migration methods:${NC}"
echo -e "1. ${CYAN}Direct transfer:${NC} Use Migration Assistant when first setting up macOS"
echo -e "2. ${CYAN}Time Machine:${NC} Restore from a backup (most comprehensive)"
echo -e "3. ${CYAN}Cloud services:${NC} iCloud Drive, Dropbox, Google Drive for files"
echo -e "4. ${CYAN}Manual sync:${NC} For selective files and configurations"
echo -e ""

read -p "$(echo -e ${CYAN}Would you like help with any specific migration task? [y/N]: ${NC})" migration_help
migration_help=${migration_help:-N}

if [[ $migration_help =~ ^[Yy]$ ]]; then
    echo -e "${GREEN}Migration tips:${NC}"
    
    echo -e "${YELLOW}For code and project files:${NC}"
    echo -e "• Use Git for version-controlled projects (push to remote first)"
    echo -e "• For non-versioned code, use a secure file transfer method"
    
    echo -e "${YELLOW}For application settings:${NC}"
    echo -e "• Many apps store settings in ~/Library/Application Support/"
    echo -e "• Carefully copy these directories (after closing the applications)"
    
    echo -e "${YELLOW}For databases:${NC}"
    echo -e "• Export database dumps before migration"
    echo -e "• Document connection details and credentials"
    
    echo -e "${YELLOW}For credentials and secrets:${NC}"
    echo -e "• Use Bitwarden for passwords, API keys, and tokens"
    echo -e "• Migration is a good time to rotate credentials"
    
    echo -e "${YELLOW}For system preferences:${NC}"
    echo -e "• Most macOS preferences are stored as .plist files in ~/Library/Preferences/"
    echo -e "• Consider using 'mackup' to back up and restore application settings"
    
    # Offer to install mackup for app settings backup
    read -p "$(echo -e ${CYAN}Would you like to install mackup to help with application settings backup? [y/N]: ${NC})" install_mackup
    install_mackup=${install_mackup:-N}
    
    if [[ $install_mackup =~ ^[Yy]$ ]]; then
        if ! brew list mackup &>/dev/null; then
            echo -e "${GREEN}Installing mackup...${NC}"
            brew install mackup
            echo -e "${GREEN}For instructions on using mackup, see: https://github.com/lra/mackup${NC}"
        else
            echo -e "${GREEN}Mackup is already installed.${NC}"
        fi
    fi
fi

# Remind about cloud services
echo -e "${YELLOW}Remember to sign in to your cloud services for seamless sync:${NC}"
echo -e "□ iCloud        □ Google Drive        □ Dropbox        □ OneDrive"
#!/bin/bash
set -e
